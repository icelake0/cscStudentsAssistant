			...
			<div id="access">
				<div id="profileNav">
					<ul>
						...
						<li><a href="textbook.php"><span style="background-color: black; padding: 4px;">TextBooks |</a></span></li>
						...
						<?php
								 ...
								 $dbname = "cscSAS";
										$conn = mysqli_connect($dbhost,$dbuser,$dbpass,$dbname) or die("Cannot Connect to the Mysql Database. Please try again later!");
											  if($conn){
											  $sql="SELECT bookname FROM csctextbooks ";
											  $result = mysqli_query($conn, $sql)or die("Can not select from table 1");
											  if (mysqli_num_rows($result) > 0) {
												while($row = mysqli_fetch_assoc($result)) {
												foreach($row as $x => $x_value) {
																		echo "<tr align='center'><td id='bookname' width='70%'>",$x_value,"</td><td width='30%'><a href='books/".$x_value."' target='_BLANK'><img width='100px'src='downloadimage.png'></a></td></tr>";							
																}
											  }
											  }
}
